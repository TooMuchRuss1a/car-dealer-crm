...
const form = useForm({
    name: props.carRequest.name,
    phone: props.carRequest.phone,
    comment: props.carRequest.comment,
});
...
const update = () => {
    form.transform((data) => ({
        ...
        .put(route('crm.requests.update', [props.carRequest.id]), {
        onSuccess: () => {
                                ...
                                <div class="card flex flex-column md:flex-row gap-3">
                                    <div class="p-inputgroup">
                                        <span class="p-float-label" v-tooltip="form.hasErrors ? form.errors.name : ''" >
                                            <InputText v-model.trim="form.name" v-bind:disabled="form.processing" :class="{'p-invalid': form.hasErrors && form.errors.name}"/>
                                            <label>Имя</label>
                                        </span>
                                    </div>
                                    <div class="p-inputgroup">
