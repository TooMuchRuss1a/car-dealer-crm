...
const form = useForm({
    ...
    car_id: props.order.car,
    customer_id: props.order.customer,
    contract_signed_at: props.order.contract_signed_at !== null,
    car_transferred_at: props.order.car_transferred_at !== null,
    payment_received_at: props.order.payment_received_at !== null,
...
const update = () => {
    form.transform((data) => ({
        ...
        contract_signed_at: data.contract_signed_at && props.order.contract_signed_at === null ? moment().format('YYYY-MM-DD H:m:s') : props.order.contract_signed_at,
        car_transferred_at: data.car_transferred_at && props.order.car_transferred_at === null ? moment().format('YYYY-MM-DD H:m:s') : props.order.car_transferred_at,
        payment_received_at: data.payment_received_at && props.order.payment_received_at === null ? moment().format('YYYY-MM-DD H:m:s') : props.order.payment_received_at
    }))
        .put(route('crm.orders.update', [props.order.id]), {
        onSuccess: () => {
            toast.add({ severity: 'success', summary: 'Успешно', detail: 'Заказ обновлен', life: 3000 });
        },
        onError: () => {
                            ...
                            <div class="space-y-8">
                                ...
                                <div class="card flex flex-column md:flex-row sm:gap-3 gap-5">
                                    <label class="flex items-center">
                                        <Checkbox v-model:checked="form.contract_signed_at" name="contract_signed_at" :disabled="props.order.contract_signed_at" />
                                        <span class="ml-2 text-sm text-gray-600">Договор подписан</span>
                                    </label>
                                    <label class="flex items-center">
                                        <Checkbox v-model:checked="form.car_transferred_at" name="car_transferred_at" :disabled="props.order.car_transferred_at" />
                                        <span class="ml-2 text-sm text-gray-600">Авто передано</span>
                                    </label>
                                    <label class="flex items-center">
                                        <Checkbox v-model:checked="form.payment_received_at" name="payment_received_at" :disabled="props.order.payment_received_at" />
